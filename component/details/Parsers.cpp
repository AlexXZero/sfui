...
sf::Color sfui::ParseColor(const nlohmann::json& json)
{
    ...
    if (json.is_number()) {
        ...
        if (named_colors.count(string)) {
            return sf::Color(named_colors.at(string));
        } else if (string.length() > 1 && string[0] == '0') {
            if (string[1] == 'x') {
                return sf::Color(std::stoul(string.substr(2), nullptr, 16));
            } else if (string[1] == 'b') {
                return sf::Color(std::stoul(string.substr(2), nullptr, 2));
            } else {
                return sf::Color(std::stoul(string.substr(1), nullptr, 8));
            }
        } else {
            return sf::Color(std::stoul(string, nullptr, 10));
        }
    }
}
